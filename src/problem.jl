...
function genproblem_static(;N::Int64=10, K::Int64=4, σm::Float64=0.05, r::Float64=0.2)
    # generate shape lib
    meanShape = @MMatrix randn(3, N)
    meanShape .-= mean(meanShape,dims=2)
    
    shapes = @MArray zeros(3,N,K)
    for k = 1:K
        shapes[:,:,k] = meanShape + r * randn(3,N)
    end
    B = reshape(shapes, 3*N, K)
    ...
    # convert to measurements
    shape = reshape(B*c, (3,N))
    y = SMatrix{3,N}((R*shape .+ p .+ σm*randn(3,N))...)
